...
<!-- #region slideshow={"slide_type": "fragment"} cell_style="split" -->
![](img/bad1.png)
<!-- #endregion -->
...
<!-- #region slideshow={"slide_type": "subslide"} hide_input=false -->
## Some ideas
...
<!-- #region slideshow={"slide_type": "subslide"} cell_style="split" -->
### 1) Check the data
...
```{python slideshow={'slide_type': 'fragment'}, cell_style="split"}
...
<!-- #endregion -->
```{python a1, eval=FALSE, cell_style="split", slideshow={'slide_type': 'fragment'}}
sns.displot(
...
```
```{python a2, eval=FALSE, cell_style="split", slideshow={'slide_type': 'fragment'}}
sns.displot(
    data = penguins,     # DataFrame
    x = 'bill_length_mm',# columns to encode
    kind = 'hist',       # Type of encoding
...
## Histograms
<!-- #endregion -->
```{python a3, eval=FALSE, cell_style="split", slideshow={'slide_type': 'fragment'}}
sns.displot(
    data = penguins,     # DataFrame
    x = 'bill_length_mm',# columns to encode
    kind = 'hist',       # Type of encoding
...
```
```{python a4, eval=FALSE, slideshow={'slide_type': 'fragment'}, cell_style="split"}
sns.displot(
    data = penguins,     # DataFrame
...
```{python slideshow={'slide_type': 'subslide'}, cell_style="split"}
sns.relplot(
    data = penguins,
    x = 'bill_length_mm',
    y = 'body_mass_g',
...
```{python tags=c(), slideshow={'slide_type': 'subslide'}, cell_style="center"}
sns.relplot(
    data = penguins,
    x = 'bill_length_mm',
    y = 'body_mass_g',
...
```{python tags=c(), slideshow={'slide_type': 'subslide'}, cell_style="split"}
sns.relplot(data = fmri, x = 'timepoint', y = 'signal', kind = 'line');
```

```{python tags=c(), slideshow={'slide_type': 'fragment'}, cell_style="split"}
